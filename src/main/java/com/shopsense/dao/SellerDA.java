...
public class SellerDA {
	...
	public Seller findByEmail(String email) throws UsernameNotFoundException {
		...
		try {
			pst = db.get().prepareStatement(
					"SELECT id, name, store_name, office_address, email, role, password FROM sellers WHERE email = ? AND status = 'Active'");
	...
	public Seller login(Seller a) {
		...
		try {
			pst = db.get().prepareStatement(
					"SELECT id, name, store_name, office_address, email, role FROM sellers WHERE email = ? AND password = ? AND status = 'Active'");
	...
	public Seller getSeller(int id) {
		...
		try {
			pst = db.get().prepareStatement(
					"SELECT name, store_name, office_address, email, balance, holder_name, account_number, bank_name, branch_name, role FROM sellers WHERE id = ?");
			pst.setInt(1, id);
			ResultSet rs = pst.executeQuery();
			if (rs.next()) {
				seller = new Seller();
	...
	public List<Product> getProducts(int sellerId) {
		...
		try {
			pst = db.get().prepareStatement(
					"SELECT products.title, thumbnail_url, products.description, regular_price, sale_price, categories.title, stock_status, stock_count, status, product_id FROM products JOIN categories ON category = category_id WHERE id = ?");
	...
	public Product createProduct(Product a) {
		try {
			pst = db.get().prepareStatement(
					"INSERT INTO products (title, thumbnail_url, description, regular_price, sale_price, category, stock_status, stock_count, id, status)"
							+ "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
	...
	public List<Order> getOrders(int id) {
		try {
			pst = db.get().prepareStatement(
					"SELECT DISTINCT order_id, order_date, order_total, customer_id, discount, shipping_charge, tax, shipping_street, shipping_city, shipping_post_code, shipping_state, shipping_country, orders.status, orders.sub_total, payment_status, payment_method, card_number, card_cvv, card_holder_name, card_expiry_date, gateway_fee FROM orders JOIN order_details USING(order_id) WHERE id = ? ORDER BY order_id DESC");
			pst.setInt(1, id);
			ResultSet rs = pst.executeQuery();
		...
		try {
			...
			if (rs.next()) {
				...
				PreparedStatement pst2 = db.get().prepareStatement(
						"SELECT order_details_id, order_id, product_id, id, store_name, product_name, product_unit_price, product_thumbnail_url, status, quantity, sub_total, delivery_date FROM order_details WHERE order_id = ? AND id = ?");
	...
	public boolean updateOrder(OrderDetails o) {
		...
		try {
			...
			if (o.getStatus().equals("Delivered")) {
				...
				pst = db.get().prepareStatement(
						"INSERT INTO revenue_profit (id, order_id, order_date, order_details_id, revenue, costs, platform_profit, seller_profit) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
				...
				pst = db.get().prepareStatement("UPDATE sellers SET balance = balance + ? WHERE id = ?");
